...
function myRows(){const query=String($('myGallerySearch')?.value||'').trim().toLowerCase(),kind=$('myGalleryKind')?.value||'',domain=$('myGalleryDomain')?.value||'';return state.charts.filter(isLocal).filter(chart=>(!query||[chart.title,chart.domain,kindLabel(chartKind(chart))].join(' ').toLowerCase().includes(query))&&(!kind||chartKind(chart)===kind)&&(!domain||chart.domain===domain)).sort((left,right)=>String(right.updated_at||'').localeCompare(String(left.updated_at||'')))}
