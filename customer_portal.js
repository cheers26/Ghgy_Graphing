...
function safeLocalCharts(){try{const rows=JSON.parse(localStorage.getItem('yd-local-charts')||'[]');return Array.isArray(rows)?rows:[]}catch{return[]}}
function safeNotes(){try{const value=JSON.parse(localStorage.getItem('yd-chart-notes')||'{}');return value&&typeof value==='object'&&!Array.isArray(value)?value:{}}catch{return{}}}
async function persistVerified(key,value){const text=typeof value==='string'?value:JSON.stringify(value);workspaceSetItem(key,text);if(window.GHGY_WORKSPACE?.setItem)await window.GHGY_WORKSPACE.setItem(key,text)}
function renderLocalFigure(frame,chart){const figure=chart.figure||chart.interactive_figure;if(figure)frame.srcdoc=localFigureDocument({...chart,figure},false)}
function myRows(){const query=String($('myGallerySearch')?.value||'').trim(),kind=$('myGalleryKind')?.value||'',domain=$('myGalleryDomain')?.value||'',eligible=state.charts.filter(isLocal).filter(chart=>(!kind||chartKind(chart)===kind)&&(!domain||chart.domain===domain)),fields=chart=>[chart.title,chart.original_title,chart.domain,kindLabel(chartKind(chart))],rows=window.GHGY_FUZZY_SEARCH?window.GHGY_FUZZY_SEARCH.filter(eligible,query,fields):eligible.filter(chart=>!query||fields(chart).join(' ').toLowerCase().includes(query.toLowerCase()));return rows.sort((left,right)=>Number(galleryOrganization(right).is_pinned)-Number(galleryOrganization(left).is_pinned)||String(right.updated_at||'').localeCompare(String(left.updated_at||''))||String(left.chart_id).localeCompare(String(right.chart_id)))}
function populateMyDomains(){const select=$('myGalleryDomain');if(!select)return;const current=select.value,domains=[...new Set(state.charts.filter(isLocal).map(chart=>chart.domain||'未分类'))].sort((a,b)=>a.localeCompare(b,'zh-CN'));select.innerHTML='<option value="">全部板块</option>'+domains.map(domain=>`<option value="${escapeHTML(domain)}">${escapeHTML(domain)}</option>`).join('');select.value=domains.includes(current)?current:''}
function localPreview(chart){const figure=chart.figure||chart.interactive_figure;if(figure)return`<iframe data-my-local-chart="${escapeHTML(chart.chart_id)}" title="${escapeHTML(chart.title)}"></iframe>`;if(chart.png_url)return`<img src="${escapeHTML(chart.png_url)}" loading="lazy" decoding="async" alt="${escapeHTML(chart.title)}">`;return'<div class="indicator-meta customer-empty-preview">本机草稿暂无预览，可进入临时试画重新生成。</div>'}
...
function renderMyGallery(){
  populateMyDomains();const rows=myRows(),pages=Math.max(1,Math.ceil(rows.length/pageSize));myPage=Math.min(myPage,pages);const visible=rows.slice((myPage-1)*pageSize,myPage*pageSize);$('myGalleryCount').textContent=`${rows.length} 张 · 第 ${myPage}/${pages} 页`;
  $('myGalleryGrid').innerHTML=visible.map(chart=>{const organization=galleryOrganization(chart),tags=myOrganizationTags(chart);return`<article class="gallery-card customer-local-card${isSeasonalMode(chart.config?.chart_mode)?' seasonal-card':''}" data-open-my-chart="${escapeHTML(chart.chart_id)}"><span class="customer-origin-badge local">我的草稿</span>${localPreview(chart)}<div class="gallery-name">${escapeHTML(chart.title)}</div>${tags?`<div class="gallery-organization-tags-v2 category-chip-line-v154">${tags}</div>`:''}<div class="gallery-organization-toolbar-v2"><button type="button" class="gallery-pin-v2" data-pin-my-chart="${escapeHTML(chart.chart_id)}" data-pinned="${organization.is_pinned}">${organization.is_pinned?'★ 已置顶':'☆ 置顶'}</button><button type="button" class="gallery-classify-v2" data-classify-my-chart="${escapeHTML(chart.chart_id)}">分类</button></div><div class="gallery-meta">${kindLabel(chartKind(chart))} · ${escapeHTML(chart.domain||'未分类')} · 本机更新 ${escapeHTML(String(chart.updated_at||'—').slice(0,16).replace('T',' '))}</div><div class="gallery-actions"><span><button class="text-button" data-view-my-chart="${escapeHTML(chart.chart_id)}">放大查看</button> · <button class="text-button" data-edit-my-chart="${escapeHTML(chart.chart_id)}">继续编辑</button> · <button class="text-button customer-delete-draft" data-delete-my-chart="${escapeHTML(chart.chart_id)}">删除本机草稿</button></span><label class="gallery-check"><input type="checkbox" data-my-showcase="${escapeHTML(chart.chart_id)}" ${state.showcase.has(chart.chart_id)?'checked':''}>加入展示面板</label></div></article>`}).join('')||'<div class="customer-empty-gallery"><strong>“我的图库”还是空的</strong><span>在“临时试画”中保存图片后，会存入当前浏览器并显示在这里。</span></div>';
  ...
  $('myGalleryGrid').querySelectorAll('iframe[data-my-local-chart]').forEach(frame=>{const chart=state.charts.find(item=>item.chart_id===frame.dataset.myLocalChart);if(chart)renderLocalFigure(frame,chart)});
...
async function deleteLocalChart(chartId){
  ...
  const charts=safeLocalCharts().filter(item=>item.chart_id!==chartId),notes=safeNotes();delete notes[chartId];const history=(()=>{try{return JSON.parse(localStorage.getItem('yd-chart-range-history')||'[]')}catch{return[]}})().filter(item=>item.chart_id!==chartId);state.showcase.delete(chartId);
  try{await persistVerified('yd-local-charts',charts);await persistVerified('yd-chart-notes',notes);await persistVerified('yd-chart-range-history',history);persistShowcase();await persistVerified('yd-showcase',[...state.showcase])}catch(error){return showMessage(`删除未完成：${error.message}`,true)}
...
async function copyBuiltInWithNote(chartId,noteText,statusElement){
  ...
  const id=`LOCAL-CH-${Date.now()}-${Math.random().toString(16).slice(2,8)}`,now=new Date().toISOString(),title=String(source.title||'未命名图表').includes('客户草稿')?source.title:`${source.title||'未命名图表'} · 客户草稿`,copy={...cloneChart(source),chart_id:id,title,status:'browser_draft',access_level:'browser_local',creation_source:'browser_local',config:{...cloneChart(source.config||{}),chart_id:id,title,status:'browser_draft',access_level:'browser_local'},html_url:null,updated_at:now},notes=safeNotes();notes[id]={note_text:String(noteText||''),version:1};
  await persistVerified('yd-local-charts',[...safeLocalCharts(),copy]);await persistVerified('yd-chart-notes',notes);await reloadCharts();if(statusElement)statusElement.textContent='已另存到“我的图库” · 说明 V1';showMessage('内置图片未修改；已另存为“我的图库”本机草稿。');switchView('my-gallery');return{chart_id:id,note_text:String(noteText||''),version:1}
}
function cloneChart(value){return value==null?value:JSON.parse(JSON.stringify(value))}

renderGallery=function(){const all=state.charts;state.charts=all.filter(isBuiltIn);try{baseRenderGallery();$('galleryGrid').querySelectorAll('[data-open-chart]').forEach(card=>{const chart=all.find(item=>item.chart_id===card.dataset.openChart);if(chart){addBadge(card,chart);const edit=card.querySelector('[data-edit-chart]');if(edit)edit.textContent='另存试画'}})}finally{state.charts=all}};
...
for(const id of['myGallerySearch','myGalleryKind','myGalleryDomain'])$(id)?.addEventListener(id==='myGallerySearch'?'input':'change',()=>{myPage=1;renderMyGallery()});
