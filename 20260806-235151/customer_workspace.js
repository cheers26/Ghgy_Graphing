...
const SCHEMA_VERSION='ghgy-workspace-v1';
...
async function buildWorkspaceModel(){
  ...
  const model={schema_version:SCHEMA_VERSION,generated_at:nowISO(),derived_drafts:derived,chart_drafts:charts,custom_domains:parseArray('yd-custom-domains').map(cleanText).filter(Boolean),showcase:{selected,order,layout,public_dependencies:publicDependencies},client_meta:{origin:location.origin,browser:navigator.userAgent.replace(/\([^)]*\)/g,'').slice(0,160)}};
...
async function packageFiles(model){
  ...
  const manifest={schema_version:SCHEMA_VERSION,generated_at:model.generated_at,content_digest:contentDigest,counts:{derived_drafts:model.derived_drafts.length,chart_drafts:model.chart_drafts.length,chart_notes:model.chart_drafts.filter(chart=>chart.note?.text).length,custom_domains:model.custom_domains.length,showcase_items:model.showcase.selected.length},limits:{max_package_bytes:PACKAGE_LIMIT,max_file_bytes:FILE_LIMIT,max_entries:ENTRY_LIMIT},files:entries.map(({path,size,sha256})=>({path,size,sha256}))};
...
async function exportPackage(){
  ...
  const blob=zipStore(files),stamp=compactLocalTimestamp();downloadBlob(blob,`Ghgy-客户工作包-${stamp}.zip`);await setMeta('last_backup_at',nowISO());setWorkspaceStatus(`工作包已导出：${model.derived_drafts.length}个草稿指标、${model.chart_drafts.length}张草稿图。`);updateStorageUI();
}
...
async function validatePackage(file){
  ...
  const derived=parseDocument(entries,'drafts/indicators.json'),charts=parseDocument(entries,'drafts/charts.json'),showcase=parseDocument(entries,'workspace/showcase.json'),domains=parseDocument(entries,'workspace/domains.json'),meta=parseDocument(entries,'workspace/meta.json');if(!Array.isArray(derived)||!Array.isArray(charts)||!Array.isArray(domains))throw new Error('工作包内容类型不正确。');assertKeys(showcase,['selected','order','layout','public_dependencies'],'showcase');
  const payload={manifest,derived,charts,showcase,domains,meta};const text=stableJSON(payload);if(TECHNICAL_ID.test(text)){TECHNICAL_ID.lastIndex=0;throw new Error('工作包包含伪造内部身份或公开运行关联键。')}TECHNICAL_ID.lastIndex=0;if(/<\s*script\b|javascript\s*:|onerror\s*=|onload\s*=/i.test(text))throw new Error('工作包包含脚本载荷。');return payload;
}
...
async function applyImport(mode){
  ...
  const incoming=materializeImport(pendingImport),current={derived:parseArray('ghgy-local-derived-drafts-v1'),charts:parseArray('yd-local-charts'),notes:parseObject('yd-chart-notes'),domains:parseArray('yd-custom-domains'),showcase:parseArray('yd-showcase')},next=mode==='replace'?incoming:{derived:[...current.derived,...incoming.derived],charts:[...current.charts,...incoming.charts],notes:{...current.notes,...incoming.notes},domains:[...new Set([...current.domains,...incoming.domains])],showcase:[...new Set([...current.showcase,...incoming.showcase])],order:[...parseArray('yd-showcase-layout-order-v1'),...incoming.order],layout:{...parseObject('yd-showcase-layout-draft-v2'),...incoming.layout}};
  const values={'ghgy-local-derived-drafts-v1':next.derived,'yd-local-charts':next.charts,'yd-chart-notes':next.notes,'yd-custom-domains':next.domains,'yd-showcase':next.showcase,'yd-showcase-layout-order-v1':next.order||incoming.order,'yd-showcase-layout-draft-v2':next.layout||incoming.layout};for(const[key,value]of Object.entries(values)){const text=JSON.stringify(value);localStorage.setItem(key,text);await idbPut(RECORD_STORE,key,text)}await setMeta('last_restore_at',{at:nowISO(),mode,schema_version:SCHEMA_VERSION});setWorkspaceStatus(`恢复完成：新增${incoming.derived.length}个草稿指标、${incoming.charts.length}张草稿图。页面将刷新。`);setTimeout(()=>location.reload(),600);
}
async function previewFile(file){pendingImport=await validatePackage(file);const counts=pendingImport.manifest.counts||{};setWorkspaceStatus(`预览通过：${counts.derived_drafts||0}个草稿指标、${counts.chart_drafts||0}张草稿图、${counts.chart_notes||0}份说明、${counts.showcase_items||0}个展示项。尚未写入。`);if(byId('workspaceMergeRestore'))byId('workspaceMergeRestore').disabled=false;if(byId('workspaceReplaceRestore'))byId('workspaceReplaceRestore').disabled=false}
